...
//  printf("calling bgc()\n");//added by Y.He to test
	...
	double annmaxplai,annet,annoutflow,anngpp,annnpp,annnbp, annprcp,anntavg;
...
//	printf("done firstday\n");//added by Y.He to test
	...
	for (simyr=0 ; ok && simyr<tmpyears ; simyr++)
	{
		...
		for (yday=0 ; ok && yday<365 ; yday++)
		{
			...
			//test by Y.He Oct/29/14
			//printf( "soilTobs=%f\t soilWobs=%f\n",metv.soilTobs,ws.soilWobs);
			...
			/* daily litter and soil decomp and nitrogen fluxes - changed from metv.tsoil to metv.soilTobs */
			if (ok && decomp(metv.soilTobs,&epc,&epv,&sitec,&cs,&cf,&ns,&nf,&nt))
			{
			...
			if (mode == MODE_MODEL)
			{
				if (ok && daily_allocation(&cf,&cs,&nf,&ns,&epc,&epv,&nt,&ws,&phen,1.0,MODE_MODEL)) //add &ws by Yuting, 06/24/2014, 11/29/2014
				{
			...
			else if (mode == MODE_SPINUP)
			{
				...
				{
					if (ok && daily_allocation(&cf,&cs,&nf,&ns,&epc,&epv,&nt,&ws,&phen,naddfrac,MODE_SPINUP))
					{
						bgc_printf(BV_ERROR, "Error in daily_allocation() from bgc.c\n");
						ok=0;
					}
				...
				{
					if (ok && daily_allocation(&cf,&cs,&nf,&ns,&epc,&epv,&nt,&ws,&phen,1.0,MODE_MODEL))
					{
						bgc_printf(BV_ERROR, "Error in daily_allocation() from bgc.c\n");
						ok=0;
					}
			...
			if (mode == MODE_MODEL)
			{
				...
				anngpp += summary.daily_gpp * 1000.0;
				annprcp += metv.prcp;
		...
		if (mode == MODE_MODEL && bgcout->bgc_ascii)
		{
			...
			fprintf(bgcout->anntext.ptr,"%6d%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f%10.1f\n",
				ctrl.simstartyear+simyr,annprcp,anntavg,annmaxlai,annet,annoutflow,annnpp,annnbp,anngpp);
		}
